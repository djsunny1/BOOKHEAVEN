// Admin Payment Options Functionality with Direct File Access
        ...
        confirmDelete.addEventListener('click', async function() {
            if (paymentOptionToDelete) {
                try {
                    ...
                    // Find and remove the payment option
                    const index = paymentOptions.findIndex(option => option.id === paymentOptionToDelete);
                    if (index !== -1) {
                        paymentOptions.splice(index, 1);
                        
                        // Save updated payment options
                        await savePaymentOptionsToFile(paymentOptions);
        ...
        paymentOptionForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (id) {
                    ...
                    const index = paymentOptions.findIndex(option => option.id === id);
                    if (index !== -1) {
...
// Function to get payment options from file
...
// Function to save payment options to file
async function savePaymentOptionsToFile(paymentOptions) {
    try {
        // Use the fetch API to send a POST request to a server-side script
        const response = await fetch('/backend/api/save-payment-options.php', {
            method: 'POST',
            headers: {
        ...
        if (!result.success) {
            throw new Error(result.error || 'Failed to save payment options');
        }
        
        return true;
    } catch (error) {
        console.error('Error saving payment options to file:', error);
        return false;
    }
}
